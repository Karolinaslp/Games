...
public class Game extends Application implements GameScreen {
    ...
    public void start(Stage primaryStage) {
        ...
        primaryStage.setTitle("CodeGym Game");
        ...
        for(int y = 0; y < this.height; ++y) {
            for(int x = 0; x < this.width; ++x) {
                ...
                if (this.showGrid && children.size() > 0) {
                    ...
                    cell.setStroke(this.toFXColor(snake.codegym_classes.Color.BLACK));
                }
    ...
    private void createScorePanel() {
        ...
        this.scoreText.setFill(this.toFXColor(snake.codegym_classes.Color.BLACK));
        StackPane scorePane = new StackPane(new Node[]{this.scoreText});
        scorePane.setBorder(new Border(new BorderStroke[]{new BorderStroke(this.toFXColor(snake.codegym_classes.Color.BLACK), BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)}));
        scorePane.setLayoutY((double)(this.height * cellSize + 110 + 6));
        int scoreHeight = 20;
        Rectangle rectangle;
        if (this.showGrid) {
            rectangle = new Rectangle((double)((this.width * cellSize - 1) / 2), (double)scoreHeight, this.toFXColor(snake.codegym_classes.Color.WHITE));
            scorePane.setLayoutX((double)(125 + (this.width * cellSize - 1) / 4));
        } else {
            rectangle = new Rectangle((double)(this.width * cellSize / 2), (double)scoreHeight, this.toFXColor(snake.codegym_classes.Color.WHITE));
    ...
    public void setCellColor(int x, int y, snake.codegym_classes.Color color) {
        if (color != null && color != snake.codegym_classes.Color.NONE) {
            ObservableList<Node> children = this.cells[y][x].getChildren();
    ...
    public snake.codegym_classes.Color getCellColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 0) {
            Color color = (Color)((Rectangle)children.get(0)).getFill();
            return this.fromFXColor(color);
        } else {
            return snake.codegym_classes.Color.NONE;
        }
    }
    ...
    public void setCellTextColor(int x, int y, snake.codegym_classes.Color color) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    public snake.codegym_classes.Color getCellTextColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    private snake.codegym_classes.Color fromFXColor(Color color) {
        ...
        Optional<Map.Entry<String, Color>> optionalColorEntry = colors.entrySet().stream().filter((entry) -> {
            return color.equals(entry.getValue());
        }).findFirst();
        snake.codegym_classes.Color[] result = new snake.codegym_classes.Color[]{snake.codegym_classes.Color.NONE};
        optionalColorEntry.ifPresent((entry) -> {
            result[0] = snake.codegym_classes.Color.valueOf((String)entry.getKey());
        });
        return result[0];
    }

    private Color toFXColor(snake.codegym_classes.Color color) {
        if (color == snake.codegym_classes.Color.NONE) {
    ...
    public void setCellValueEx(int x, int y, snake.codegym_classes.Color cellColor, String value) {
        this.setCellValue(x, y, value);
        this.setCellColor(x, y, cellColor);
    }

    public void setCellValueEx(int x, int y, snake.codegym_classes.Color cellColor, String value, snake.codegym_classes.Color textColor) {
        this.setCellValueEx(x, y, cellColor, value);
        this.setCellTextColor(x, y, textColor);
    }

    public void setCellValueEx(int x, int y, snake.codegym_classes.Color cellColor, String value, snake.codegym_classes.Color textColor, int textSize) {
        this.setCellValueEx(x, y, cellColor, value, textColor);
        this.setCellTextSize(x, y, textSize);
    }

    public void showMessageDialog(snake.codegym_classes.Color cellColor, String message, snake.codegym_classes.Color textColor, int textSize) {
    ...
    public void setLives(int lives) {
        if (this.livesText == null) {
            ...
            this.livesText.setFill(this.toFXColor(snake.codegym_classes.Color.ORANGE));
